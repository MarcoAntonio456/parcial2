...
public class EstudianteDAOimpl extends ConexionDB implements EstudianteDAO{
    ...
    public void insert(Estudiante estudiante) throws Exception {
        try{
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("INSERT into estudiantes(nombres, apellidos, seminario, confirmado, fecha_inscripcion) values (?, ?, ?, ?, ?)");
    ...
    public void update(Estudiante estudiante) throws Exception {
        try{
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("UPDATE estudiantes set nombres = ?, apellidos = ?, seminario = ?, confirmado = ?, fecha_inscripcion = ? where id = ?");
            ps.setString(1, estudiante.getNombres());
            ps.setString(2, estudiante.getApellidos());
            ps.setString(3, estudiante.getSeminario());
            ps.setBoolean(4, estudiante.isConfirmado());
